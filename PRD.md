...
|------|------|
| 书名/作者 | 书源获取，可用户自定义 |
| 评分 | 用户自定义评分，1-5分，默认1分，在书籍信息页通过RatingBar设置 |
| 标签 | 用户自定义标签，逗号分隔，在书籍信息页通过EditText编辑 |
| 封面 | 书源获取，可用户自定义 |
| 简介 | 书源获取，可用户自定义 |
| 分类标签 | 书源获取 + 用户自定义 |
| 未读章节数 | 自动计算 |
...
**评分功能：**
- 评分共5级，范围1-5分，默认1分
- 在书籍信息页面通过RatingBar（星级评分条）设置评分
- 评分支持stepSize=1，只能选择整数分
- 评分的修改仅在书籍已加入书架时保存到数据库

**标签功能：**
- 支持用户自定义标签，多个标签以逗号分隔
- 在书籍信息页面通过EditText输入框编辑标签
- 标签焦点离开时自动保存（仅已加入书架的书籍）

**书架评分显示：**
- 在书架列表页面，作者信息下方显示评分
- 显示格式为"评分 x 分"（中文）/ "Rating x points"（英文）
- 使用星形图标（ic_star_border）作为评分标识

**按评分排序：**
- 书架管理排序选项第7项为"按评分"排序
- 支持正序（1→5分）和反序（5→1分）排列
...
| order | Int | 手动排序 |
| rating | Int | 用户评分（1-5，默认1） |
| tags | String | 用户自定义标签（逗号分隔） |
...
- 3.26+：新增书籍评分功能（1-5分）、书籍标签功能、按评分排序、书架评分显示
- 持续迭代中，详见更新日志